...
def sendMessage(subject="NO SUBJECT", content="NO VALUE"):
    ...
    directSend(deviceName + separator + subject + separator + content)
    ...
    while (not finishedNetIN):  # loop until execution halted
        ...
        while True:
            ...
            message = data.decode('utf-8',
                                  'replace')  # byte data has to be converted into a character set - 'utf-8' here. 'replace' replaces characters it does not recognise. Was causing comparison prolems "exit" == "exit" was not working. Would show letter 'b' before printing on OLED to denote was raw bytes and hadn't been decoded yet.
...
# ==================================
# sends one message
def directSend(messageString):
    """sends one message - no buffer"""
    ...
    client_socket.sendall(str(messageString).encode())  # sends nextMessage to game server encoded in (UTF-8?)
...
# ==================================
# uses buffer for stacking messages. For some reason would stop working when buffer went to zero.
# haven't isolated problem yet.
def networkOUT():
    ...
    while not finishedRFID:
        ...
        try:
            ...
            if (str(id) != previousSquare):  # throttling - reduces sending messages over network to only when RFID reading different to previous
                ...
                currentSquare = str(id)        # tag serial number
...
# network out
#--------------
# used with sending messages using buffer - but had problems with the stack going to zero and then struggling to restart.
